...
public final class Cmplx {
    ...
    /** Returns the complex array forward Fast Fourier Transform
     *   of a float array.  The complex array will have a length that is
     *   the smallest power of 2 greater than or equal to the length
     *   of the float array.
    ...
    public static final double[] Spectro(double[] fdata, int nPointsPow2) {
        ...
        // create complex array
        double[] cdata = new double[nPointsPow2/2];
        double temp;
        int nb = nPointsPow2/2;
        
        // spectra
        for (i = 0, j = 0; i < nPointsPow2/2; i++, j += 2) {
        	temp = 10 * (double)Math.log10((data[j]*data[j] + data[j+1]*data[j+1]));
        	cdata[i] = temp ;
        }
    ...
    /** Returns the float array inverse Fast Fourier Transform
     *   of a complex array.
     *
     ...
     *   of a pseudo-complex double array representing an array
     *   of complex pairs.  (adapted from Numerical Recipies in C)
     *
     * @param     data  the input double array.
     * @param     isign  FFT direction.
     *                   (isign=1: returns forward FFT of data.
     *                    isign=-1: returns inverse FFT or data.)
    ...
    *
    */
   public static void cosineTaper(double[] sample, double taperPercent) {
